    ...
    openModalButtons.forEach(button => {
        button.addEventListener("click", function () {
            modalContent.innerHTML = `
                ...
                <h2>إرسال طلب</h2>
                <input type="text" placeholder="اسمك*" id="nameInput" required>
                <input type="text" placeholder="سيزينغ تليفونينغيز*" id="phoneInput" required>
                <button id="submit">لإرسال</button>
            `;
            ...
            document.getElementById("submit").addEventListener("click", () => {
                ...
                modalContent.innerHTML = `
                    <span class="close">&times;</span>
                    <h2 style="color: #ff0; text-align: center;">تم إرسال طلبك!</h2>
                    <p style="text-align: center; color: #ff0;">سيقوم موظفنا بالاتصال بك قريبا.</p>
                `;
  ...
  document.querySelectorAll('form[data-google-form]').forEach(form => {
    ...
    form.addEventListener('submit', e => {
      ...
      fetch(scriptURL, { method: 'POST', body: formData })
        ...
        .catch(error => {
          loading.style.display = 'none';
          alert("خطأ. يرجى المحاولة مرة أخرى.");
